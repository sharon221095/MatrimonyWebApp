...
const Profile = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            try {
                const response = await fetch(`https://nrimarriage.in/api/v1/Users/GetUser?id=${id}`, {
                    method: 'GET',
                    headers: {
        ...
        const fetchUserImages = async () => {
            const token = localStorage.getItem('authToken');
            try {
                const response = await fetch(`https://nrimarriage.in/api/v1/Users/GetUserImages?userId=${id}`, {
                    method: 'GET',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json',
